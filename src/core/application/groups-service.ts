...
export async function getGroupById(c: Context<EnvAPI>) {
  ...
  const group = await GroupsDS.getByID(groupId);
...
//   const subgroupId = parseInt(c.req.param("subgroup_id"), 10);
//   const subgroup = await SubgroupsDS.getByID(subgroupId);
...
//   const subgroup = await SubgroupsDS.getByID(subgroupId);
//   if (!subgroup) return c.json({ msg: "Group not found" }, 404);
//   const parentGroup = await GroupsDS.getByID(subgroup?.parent_group_id);
...
//   const subgroupDB = await SubgroupsDS.getByID(subgroupId);
//   if (!subgroupDB) return c.json({ msg: "Group not found" }, 400);
//
//   const parentGroup = await GroupsDS.getByID(parent_group_id);
