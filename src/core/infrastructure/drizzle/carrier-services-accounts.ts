...
import { CarrierServiceAccount, UpdateCarrierServiceAccount } from "~/core/domain/carrier-service-accounts/types";
...
export class CarrierServicesAccountsDS {
  ...
  static async update(account: UpdateCarrierServiceAccount, tx?: Transaction) {
    return (await (tx || db).update(carrier_accounts).set(account).where(eq(carrier_accounts.account_id, account.account_id)).prepare().execute());
  }
