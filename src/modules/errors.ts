...
export function handleErrors(err: Error, c: Context<EnvAPI>) {
  if (err instanceof CustomError) {
    ...
    return c.json({ status: "error", msg: err.message }, err.code);
  }

  logger.error(err);
  const code = (err as any).status as number;

  if (code in ALLOWED_ERRORS) return c.json({ status: "error", msg: ALLOWED_ERRORS[code] }, code);
  return c.json({ status: "error", msg: "Internal Server Error" }, 500);
}
